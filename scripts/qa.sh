...
# La couverture n'est mesurable qu'avec Xdebug ou PCOV. Sans pilote, on lance
# les tests sans elle plutôt que d'échouer : mieux vaut un garde-fou partiel
# qu'un garde-fou contourné.
if php -r 'exit((extension_loaded("xdebug") || extension_loaded("pcov")) ? 0 : 1);'; then
    run "PHPUnit + couverture (plancher ${COVERAGE_MIN} %)" bash -c \
        "XDEBUG_MODE=coverage php vendor/bin/phpunit --coverage-clover var/clover.xml \
         && php scripts/coverage-check.php var/clover.xml ${COVERAGE_MIN}"
else
